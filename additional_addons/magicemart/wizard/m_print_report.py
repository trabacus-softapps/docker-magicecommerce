...
class print_reportwiz(osv.osv_memory):
    ...
    _columns = {
                'type'    :   fields.selection([('pdf', 'PDF'),('xl','XLS')]," Report Type"),
    ...
    def but_print(self, cr, uid, ids, context=None):
        ...
        if active_id:
            ...
            for case in self.browse(cr, uid, ids, context=context):
                if not case.type:
                     raise osv.except_osv(_('Warning!'), _('Please Select The Type Of Report to Print'))
                if case.type == "pdf":
                    rep_obj = self.pool.get('ir.actions.report.xml')
                    res = rep_obj.pentaho_report_action(cr, uid, 'Sales Quotation', [active_id],None,None)
                    res.update({'name': sale.name and 'Sales Quotation - ' + sale.name  or 'Sales Quotation'})
                    return res
                else:
    ...
    def print_purchase(self, cr, uid, ids, context=None):
        ...
        if active_id:
            ...
            for case in self.browse(cr, uid, ids, context=context):
                ...
                if case.type == "pdf":
                    rep_obj = self.pool.get('ir.actions.report.xml')
                    res = rep_obj.pentaho_report_action(cr, uid, 'Purchase Order', [active_id],None,None)
                    res.update({'name': pur.name and 'Purchase Order - ' + pur.name  or 'Purchase Order'})
                    return res
                else:
    ...
    def print_invoice(self, cr, uid, ids, context=None):
        ...
        if active_id:
            ...
            for case in self.browse(cr, uid, ids, context=context):
                ...
                if case.type == "pdf":
                    rep_obj = self.pool.get('ir.actions.report.xml')
                    res = rep_obj.pentaho_report_action(cr, uid, 'sales invoice', [active_id],{'option_sign': case.option_sign or False},None)
                    res.update({'name': invoice.number and 'sales invoice - ' + invoice.number  or 'sales invoice'})
                    return res
                else:
    ...
    def print_invoice1(self, cr, uid, ids, context=None):
        ...
        if active_id:
            ...
            for case in self.browse(cr, uid, ids, context=context):
                ...
                if case.type == "pdf":
                    rep_obj = self.pool.get('ir.actions.report.xml')
                    res = rep_obj.pentaho_report_action(cr, uid, 'Sales Invoice1', [active_id],{'option_sign': case.option_sign or False},None)
                    res.update({'name': invoice.number and 'Sales InvoiceMagic ecommerce Pvt. Ltd.,1 - ' + invoice.number  or 'Sales Invoice1'})
                    return res
                else:
    ...
    def print_deliverychallan(self, cr, uid, ids, context=None):
        ...
        if active_id:
            ...
            for case in self.browse(cr, uid, ids, context=context):
                ...
                if case.type == "pdf":
                    rep_obj = self.pool.get('ir.actions.report.xml')
                    res = rep_obj.pentaho_report_action(cr, uid, 'Delivery challan', [active_id],None,None)
                    res.update({'name': stock.name and 'Delivery challan - ' + stock.name  or 'Delivery challan'})
                    return res
                else:
...
#                 'fiscal_year' :   fields.many2one("account.fiscalyear", "Fiscal Year"),
                ...
                'new_sale_vat':   fields.boolean("Consolidated Sales Vat"),       
