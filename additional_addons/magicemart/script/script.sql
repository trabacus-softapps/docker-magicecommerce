	...
	select 	nextval('do_seq') as id ,
		...
		from
			...
			(SELECT sum(sm.product_qty) as qty
				...
				INNER JOIN stock_location l on l.id = sm.location_dest_id
				INNER JOIN stock_picking_type spt on spt.id = sp.picking_type_id
				AND sm.state='done'
				AND sm.location_id != sm.location_dest_id 
				AND CASE WHEN spt.code IS NOT NULL THEN spt.code!='outgoing' ELSE spt.code is null END
				AND sm.date >= fiscalst_date::date AND sm.date < from_date::date
				AND sm.product_id = prod_id 
				AND CASE WHEN compn_id > 0 THEN l.company_id = compn_id ELSE l.company_id > 0 END
		...
		union all
			 ...
			 SELECT sum(-sm.product_qty) as qty
				...
				LEFT OUTER JOIN stock_picking sp on sp.id= sm.picking_id 
				INNER JOIN product_product p on p.id = sm.product_id
				INNER JOIN stock_location l on l.id = sm.location_id
				INNER JOIN stock_picking_type spt on spt.id = sp.picking_type_id
				AND sm.state='done'
				AND CASE WHEN spt.code IS NOT NULL THEN spt.code!='incoming' ELSE spt.code is null END
				AND sm.date >= fiscalst_date::date AND sm.date < from_date::date
				AND sm.product_id = prod_id 
				AND CASE WHEN compn_id > 0 THEN l.company_id = compn_id ELSE l.company_id > 0 END
			)a
	...
	(	
		...
		SELECT 		
			...
			FROM
				(SELECT 
					  sm.date::date
					, CASE WHEN spt.code IS NOT NULL THEN (CASE WHEN spt.code = 'incoming' THEN spt.code ELSE 'incoming' END) ELSE 'incoming' END as type
					--, sp.type
				...
				AND sm.location_id != sm.location_dest_id 
				AND CASE WHEN spt.code IS NOT NULL THEN spt.code!='outgoing' ELSE spt.code is null END
				AND sm.date >= from_date::date AND sm.date <= to_date::date
				AND sm.product_id = prod_id 
				AND CASE WHEN compn_id > 0 THEN l.company_id = compn_id ELSE l.company_id > 0 END
			...
			UNION all
				...
				SELECT	  sm.date::date
					, CASE WHEN spt.code IS NOT NULL THEN (CASE WHEN spt.code = 'outgoing' THEN spt.code ELSE 'outgoing' END) ELSE 'outgoing' END as type
					--, sp.type
					, p.name_template as product
					, (-sm.product_qty) as qty
					, sp.origin
					, (select id from res_partner where id = sp.partner_id) as desc_id
					, sm.id
				FROM stock_move sm
				LEFT OUTER JOIN stock_picking sp on sp.id= sm.picking_id 
				INNER JOIN product_product p on p.id = sm.product_id
				...
				GROUP BY d.desc_id,d.date,d.type,d.origin,d.qty,d.id
